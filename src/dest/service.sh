...
start() {
  _create_config
  rm -f "${pidfile}"
  PATH="${prog_dir}/libexec:${DROBOAPPS_DIR}/git/bin:${PATH}" PYTHONPATH="${prog_dir}/lib/python2.7/site-packages" "${python}" "${prog_dir}/app/Headphones.py" --datadir="${prog_dir}/data" --pidfile="${pidfile}" --nolaunch --daemon
}
